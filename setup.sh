...
░░   ░▒ ░  ░ ▒ ▒░ ░ ░▒  ░ ░ ▒ ░░ ░░   ░ ▒░    ░    
 ...
 Version: \033[1;92m2.3
   \033[0m"
   echo
	printf "\e[1;34m\tSelect your terminal to install FIXTT \e[0m\n"
...
echo
cp -r fixtt $PREFIX/bin
cp -r subdomains.txt $PREFIX/bin
cp -r templates $PREFIX/bin
cp -r static $PREFIX/bin
cd $PREFIX/bin && chmod u+x fixtt
cd $PREFIX/bin && chmod u+x subdomains.txt
cd $PREFIX/bin && chmod u+x templates
cd $PREFIX/bin && chmod u+x static
cd $HOME && rm -rf FIXTT
printf "\e[1;91m[*]\e[1;97m FIXTT Installed Successfully, Now its ready for use. So re-open your Termux To use FIXTT\e[0m\n" 
printf "\e[1;91m[*]\e[1;97m After reopen your termux just type '\e[1;91mfixtt\e[1;97m' to launch FIXTT \e[0m" 
exit
...
sleep 1
echo
sudo cp -r fixtt /usr/local/bin
sudo cp -r subdomains.txt /usr/local/bin
sudo cp -r templates /usr/local/bin
sudo cp -r static /usr/local/bin
cd /usr/local/bin && sudo chmod +x fixtt
cd /usr/local/bin && chmod u+x subdomains.txt
cd /usr/local/bin && chmod u+x templates
cd /usr/local/bin && chmod u+x static
cd $HOME && sudo rm -rf FIXTT
echo
 printf "\e[1;91m[*]\e[1;97m FIXTT Installed Successfully, Now its ready for use. So re-open your Linux Terminal To use FIXTT\e[0m\n"
 echo
 echo
 printf "\e[1;91m[*]\e[1;97m After reopen your terminal just type '\e[1;91msudo fixtt or python fixtt or fixtt\e[1;97m' to launch FIXTT \e[0m" 
 echo ""
 exit
else 
